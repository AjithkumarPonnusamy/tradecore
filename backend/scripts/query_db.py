...
with engine.connect() as conn:
    result = conn.execute(text("SELECT symbol, market, live_price, previous_day_high, previous_day_low, previous_day_close, cpr_levels, camarilla_levels FROM market_data WHERE symbol = 'XAUUSD'"))
    for row in result:
        print("Symbol:", row[0])
        print("Market:", row[1])
